...
<div class="row">
    <div class="col-md-12 col-lg-12 mx-auto">
        ...
        <div class="card-header"><i class="fas fa-table mr-1"></i>
          <?php echo strtoupper(UID()) ?>, <small> <?php echo $data['companyName'] ?></small></h4> &nbsp;  <?php echo $data['msg'] ?>
        </div>
        <div class="table-responsive">
          ...
          <?php $n=0; foreach($data['cust'] as $row) :?>
            <tr>
              ...
              <td class="text-white <?php echo (($row->status)=='not metered') ? 'bg-warning' : 'bg-success'  ?>"><?php echo $row->status ?></td>
              <td><?php echo date('Y-m-d',strtotime($row->created_on)) ?></td>
              <td><?php echo $row->uid ?></td>
              <td><?php echo $row->dayscount ?></td>
              <td>
                <?php if(strtoupper($row->status)=='NOT INSTALL'): ?>
